...
> 偏好沉淀（2026-07-05，v1 实跑用户确认）：默认视觉方案由"完整聊天窗口累积截图"改为 **黑屏背景 + 当前一句居中大字气泡卡片**——长句按标点拆成短语逐句出现，emoji/贴纸做情绪强化。原因：完整聊天截图在手机上文字不醒目，大字卡片才有参考视频的"怼脸"冲击力。以下四个视觉要点仍然全部适用，只是承载物从"整窗截图"换成"单句大字卡片"。

方案：`remotion/src/ChatBubble.tsx` 组件按 `dialogue_APPROVED.json` 逐句渲染大字气泡卡片（黑屏背景、当前句居中放大；长句按标点拆分逐句出现），不使用完整聊天窗口截图、不使用一张长图代替全部场景。组件必须实现参考视频里出现的四个视觉要点：

1. **开场钩子帧**：`remotion/src/ChatVideo.tsx` 顶部按 `timeline.hook` 渲染一帧真实感的微信对话框（返回箭头 + 联系人名 + 第一条消息），停留时长取 `hook.duration`，之后再切入逐句大字卡片场景。
2. **拍入动画**：每条新消息用 `interpolate()` 驱动“轻微旋转 + 缩放”的进入动作（对应 `timeline` 里的 `entrance: "tilt_drop"`），不能用纯淡入或硬切代替。
3. **跟随镜头**：画面按 `scene.camera` 做竖向缩放/平移，上下动态留黑边，模拟手机竖屏录屏被二次构图，不能让画面整屏铺满静态截图。
